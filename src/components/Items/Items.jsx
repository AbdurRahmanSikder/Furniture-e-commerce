...
const Items = ({ id, image, description, new_price, old_price, discount }) => {
  return (
    <div className="max-w-sm bg-[#F4F5F7] hover:bg-slate-300 overflow-hidden cursor-pointer transition-transform duration-300 hover:scale-105 ">
      {/* Image Section */}
      <Link to={`/shop/${id}`}><div className="relative">
        <img onClick={window.scroll(0, 0)} src={image} alt={description} className="w-full h-60 object-cover" />
        {discount && (
